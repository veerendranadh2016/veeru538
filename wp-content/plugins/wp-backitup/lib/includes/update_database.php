...
function wpbackitup_update_database() {
	...
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Running database upgrade routines.');
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Current DB version:'.$current_db_ver );
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Target DB version:'.$target_db_ver );
	...
	if ($current_db_ver==0){
		...
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Updated Db version in settings:'. $target_db_ver );
	...
	while ( $current_db_ver < $target_db_ver ) {
		...
		$current_db_ver ++;
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, sprintf('Upgrading database to version %s',$current_db_ver) );
		...
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Updated Db version in settings:'. $current_db_ver );
	}
...
//	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Begin upgrade database to V1');
//	global $wpdb;
//
...
function wpbackitup_update_database_routine_3($log_name) {
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Begin upgrade database to V3');
	...
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'End upgrade database to V3');
}
...
function wpbackitup_update_database_routine_4($log_name) {
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__, 'Begin upgrade database to V4' );
	global $wpdb;
	...
	$sql = "ALTER TABLE $job_control_table
			...
			";
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'UPDATE SQL:'. $sql);

	$wpdb_result = $wpdb->query($sql);
	$last_error = $wpdb->last_error;
	if ($wpdb_result === FALSE && !empty($last_error)) {
		WPBackItUp_LoggerV2::log_error($log_name,__METHOD__,'Last Error:' .var_export( $last_error,true ) );
	}else{
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Job Name column updated successfully');
	}

	//scan the host for backups - import them into job tables
	wpbackitup_scan_import_backups($log_name); //needed because update 3 might have failed

	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'END upgrade database to V4');
}
...
function wpbackitup_create_custom_tables($log_name){
	...
	WPBackItUp_LoggerV2::log($log_name,'*** DROP JOB TABLES ***');

	//drop the original job table
	$sql = "DROP TABLE IF EXISTS $old_job_table;";
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Drop OLD Job Table:' .$sql);
	$wpdb_result = $wpdb->query($sql);
	$last_error = $wpdb->last_error;
	if ($wpdb_result === FALSE && !empty($last_error)) {
		WPBackItUp_LoggerV2::log_error($log_name,__METHOD__,'Last Error:' .var_export( $last_error,true ) );
	}else{
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'OLD Job Table Dropped successfully');
	}


	//drop the existing job table
	$sql = "DROP TABLE IF EXISTS $job_control_table;";
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Drop Job Table:' .$sql);
	$wpdb_result = $wpdb->query($sql);
	$last_error = $wpdb->last_error;
	if ($wpdb_result === FALSE && !empty($last_error)) {
		WPBackItUp_LoggerV2::log_error($log_name,__METHOD__,'Last Error:' .var_export( $last_error,true ) );
	}else{
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Job Table Dropped successfully');
	}

	//drop the existing Task table
	$sql = "DROP TABLE IF EXISTS $job_tasks_table;";
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Drop Task Table:' .$sql);
	$wpdb_result = $wpdb->query($sql);
	$last_error = $wpdb->last_error;
	if ($wpdb_result === FALSE && !empty($last_error)) {
		WPBackItUp_LoggerV2::log_error($log_name,__METHOD__,'Last Error:' .var_export( $last_error,true ) );
	}else{
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Task Table Dropped successfully');
	}
	...
	if ($wpdb_result === FALSE && !empty($last_error)) {
		...
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Item Table Dropped successfully');
	}

	WPBackItUp_LoggerV2::log($log_name,'*** END DROP JOB TABLES ***');

	WPBackItUp_LoggerV2::log($log_name,'*** CREATE JOB TABLES ***');
	...
	dbDelta( $sql );
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Table Created:'. $job_control_table);
	...
	dbDelta( $sql );
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Table Created:'. $job_tasks_table);
	...
	dbDelta( $sql );
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Table Created:'. $job_items_table);

	WPBackItUp_LoggerV2::log($log_name,'*** END CREATE JOB TABLES ***');
}
...
function  wpbackitup_scan_import_backups($log_name){
	...
	WPBackItUp_LoggerV2::log($log_name,'***SCAN & IMPORT BACKUPS ***');

	//get all the folders in the backup path
	$backup_folder_root = WPBACKITUP__BACKUP_PATH;
	$backup_folders = glob($backup_folder_root. '/*', GLOB_ONLYDIR);
	WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Backup Sets Found:' .var_export($backup_folders,true));

	//if not TMP and DLT add to job table
	foreach($backup_folders as $folder) {
		WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Importing:' .$folder);
		...
		if ('TMP_'!= strtoupper($folder_prefix) && 'DLT_'!= strtoupper($folder_prefix)){
			...
			if (false===$jobs){
				...
				if (! is_array($zip_files) || count($zip_files)<=0){
					WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'No backups found in folder:' .$folder);
					continue;
				}
				...
				if (false!== $job){
					...
					WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Update Job Meta:' .var_export($zip_files,true));
			...
			WPBackItUp_LoggerV2::log_info($log_name,__METHOD__,'Backup was not imported:' .$folder);
		}
	}

	WPBackItUp_LoggerV2::log($log_name,'***END SCAN & IMPORT BACKUPS ***');
